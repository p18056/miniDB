...
def analyse_multiple_select(query):
    ...
    #for example takes a quwery of multiple selects σθ1(σθ2(...(σθn(instructor)))) and returns [σθ1,σθ2,...,σθn],instructor
    #also if a condition has 'and' split the condition and turn the query into multiple selects 
    #for example takes σθ1^θ2^...^θn(instructor) and returns [σθ1,σθ2,...,σθn],instructor
    Query_list=[]
    From = []
...
def combine_multiple_selects(list_selects,_from):
    ...
    # for example takes the list [σθ1,σθ2,...,σθn] and one or more tables for example 'instructor' and returns σθ1(σθ2(...(σθn(instructor))))
